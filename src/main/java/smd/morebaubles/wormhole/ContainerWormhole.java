...
import smd.morebaubles.capability.CapabilityWormholePins.IWormholePins;
...
//		morebaubles.logger.info("container constructor");
		if (player.world.isRemote)
			return;
//		morebaubles.logger.info("not remote");
...
//					morebaubles.logger.info("found "+target.getName());
...
//		morebaubles.logger.info(targets.size());
...
//		morebaubles.logger.info("detect and send chang");
		super.detectAndSendChanges();
//		morebaubles.logger.info(dirty);
...
//		morebaubles.logger.info(changes);
...
//		morebaubles.logger.info("pinning "+index);
		if (index<0||index>targets.size()-1)
			return;
		IWormholeTarget target = targets.get(index);
//		morebaubles.logger.info(target);
//		morebaubles.logger.info(pinCount);
		if (index<pinCount) {
			pinCount--;
