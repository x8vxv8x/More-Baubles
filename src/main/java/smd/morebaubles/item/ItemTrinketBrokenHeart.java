...
public class ItemTrinketBrokenHeart extends AGenericItemBauble {
	...
	public ItemTrinketBrokenHeart() {
		super("trinketBrokenHeart", morebaubles.TAB);

		if (regenHearts==null) {
			morebaubles.config.addPropBoolean(getRegistryName()+".regenheartcontainers",
					"Items",
			...
			regenHearts = morebaubles.config
...
//		morebaubles.logger.info("playerhit");
...
//		morebaubles.logger.info("health after damage "+healthAfterDamage);
		if (healthAfterDamage>=1)
			return;
		double maxHealthDamage = 1D-healthAfterDamage;
//		morebaubles.logger.info("damage to maxHealth "+maxHealthDamage);
