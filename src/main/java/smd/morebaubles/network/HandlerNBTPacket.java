...
import smd.morebaubles.network.PacketHandler.HandlerIds;
import smd.morebaubles.network.wormhole.HandlerWormhole;
import smd.morebaubles.network.wormhole.HandlerWormholeClient;
import smd.morebaubles.network.wormhole.HandlerWormholeRequest;
...
public class HandlerNBTPacket implements IMessageHandler<NBTPacket, IMessage> {
	...
	private void handleMessage(NBTPacket message, MessageContext ctx) {
		...
		if (id==HandlerIds.SYNC_SERVER_DATA.id) {
			Config conf = morebaubles.config;
